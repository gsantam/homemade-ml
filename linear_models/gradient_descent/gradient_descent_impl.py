...
def gradient_descent(gamma,function,d_function,x_0 ,max_steps = 500):
    ...
    while abs(function(*x_n_1)-function(*x_n))>tol and i<max_steps and function(*x_n_1)!=float("inf") :
        ...
        x_iters.append(x_n)
        x_n = x_n_1
        x_n_1 = next_step_(gamma,d_function, x_n )
        i+=1
...
def next_step_(gamma,d_function,x):
    eval_d_function = d_function(x)
    return x - gamma*(eval_d_function) 
    

def gradient_descent_(gamma,function,d_function,x_0 ,max_steps = 500,tol = 10**-6):
    x_iters = []
    x_n = x_0
    x_n_1 = next_step_(gamma,d_function, x_n )
    i = 0 
    ...
    print("The algorithm converged in {i} iters".format(i = i))
    return x_iters
